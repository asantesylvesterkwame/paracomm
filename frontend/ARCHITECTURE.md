...
`files/message/voice-note/` is the second sub feature and follows the same shape: `voice-note.service.ts` (multipart upload, blob download of the media route, dub requests), `voice-note.constants.ts` with `VOICE_COPY`, `voice-note.interface.ts`, `voice-note.utils.ts` (the module level `pendingRecordingStore` that keeps a recorded blob alive between the optimistic bubble and the upload) and `useVoiceNote.ts`, which wraps the recorder and the redub action. Its components (`VoiceRecorderBar`, `VoiceNoteBubble`, `VoiceNotePlayer`, `DubStatus`, `RedubPicker`) live in `files/message/components/` because they render inside the conversation. Sending goes through the same `useMessage` queue as text: `sendVoiceNote` builds a `kind: "voice"` optimistic message with a local object URL, and `dispatchSend` routes by kind, so retries and the reconnect flush never resend a voice note as text. Two generic primitives sit in `hooks/`: `useVoiceRecorder` (`getUserMedia` plus `MediaRecorder`, mime negotiation, tap to start and tap to stop, a hard 60 second cap) and `useAudioPlayback` (one active `HTMLAudioElement` for the whole page, an object URL cache, per key status and progress), and `utils/audio.ts` gained `encodeWavFromBlob` for browsers that only record `audio/mp4`. Dub progress arrives on `message:updated` as the `voiceNote` field of the message payload; the bubble never polls.
